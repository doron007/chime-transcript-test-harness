...
(function () {
  ...
  const Utils = {
    ...
    fuzzyMatch: (str1, str2) => {
      ...
      // Check if one is a complete extension of the other (one begins with the other)
      if (norm1.startsWith(norm2) || norm2.startsWith(norm1)) {
        const shorterLength = Math.min(norm1.length, norm2.length);
        const longerString = norm1.length > norm2.length ? norm1 : norm2;
        const matchPercentage = shorterLength / longerString.length;

        if (
          (shorterLength >= CONFIG.FUZZY_MATCH.MIN_LENGTH ||
            matchPercentage >= CONFIG.FUZZY_MATCH.SHORT_MATCH_THRESHOLD) &&
          matchPercentage >= 0.4
        ) {
          return true;
        }
      ...
      // If strings are very short, don't rely on word-based comparison
      if (
        norm1.length < CONFIG.FUZZY_MATCH.MIN_LENGTH ||
        norm2.length < CONFIG.FUZZY_MATCH.MIN_LENGTH
      ) {
        return false;
      }
      ...
      // Check if the initial 40% of meaningful words are in common
      const initialPortion = Math.floor(
        Math.min(words1.length, words2.length) * CONFIG.FUZZY_MATCH.INITIAL_PORTION
      );
      const initialWords1 = words1.slice(0, initialPortion);
      const initialWords2 = words2.slice(0, initialPortion);
      const initialCommonCount = initialWords1.filter((word) =>
        initialWords2.includes(word)
      ).length;
      const initialSimilarity = initialCommonCount / Math.max(initialPortion, 1);

      // Require high overall similarity or high similarity in initial portion
      return (
        (similarity >= CONFIG.FUZZY_MATCH.SIMILARITY_THRESHOLD &&
          commonWords.length >= CONFIG.FUZZY_MATCH.MIN_COMMON_WORDS) ||
        (initialSimilarity >= CONFIG.FUZZY_MATCH.SIMILARITY_THRESHOLD && initialPortion >= 3)
      );
    },
  };
